...
\begin{document}
	...
	\colorbox{grey}{
		\parbox[t]{0.91\linewidth}{
			\centering \fontsize{50pt}{80pt}\selectfont % Il primo ÃÂ¨ la grandezza del font, il secondo lo spazio lasciato
	...
	\end{itemize}
	\subsection{Caso d'uso UC8.4.2.8: Visualizzazione errore descrizione API}
	\label{UC8.4.2.8}
	\begin{itemize}
		\item \textbf{Attori}: Utente Autenticato;
		\item \textbf{Scopo e descrizione}: L'utente tenta di inserire una descrizione per il microservizio più lunga di 1000 caratteri. Il sistema notifica un errore;
		\item \textbf{Precondizione}: L'utente ha inserito una descrizione troppo lunga;
		\item \textbf{Postcondizione}: Il sistema ha fallito nel caricamento e ha notificato un errore adeguato all'utente.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Scenario principale}: 
			\begin{itemize}
				\item l'utente sceglie l'importo da convertire in APICredits [UC8.5.4];
				\item l'utente visiona i dati della carta di credito salvata e conferma la loro correttezza [UC8.5.1];
				\item l'utente conferma il pagamento [UC8.5.2].
			\end{itemize}
