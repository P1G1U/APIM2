...
\hline
Stesura finale & Sebastiano Marchesini & 04/01/2016 & 0.1.2 \\
\hline
Inserimento Tabelle & Sebastiano Marchesini & 05/01/2016 & 0.1.3 \\
\hline
\end{tabular}
}
\normalsize
...
Il generale scopo del documento è di misurare l'efficienza e tenerla in considerazione preventivamente. Importantissimo per l committente che tiene d'occhio della stima delle risorse. \\
E' in particolare una dichiarazione di interfaccia di pianificazione e consuntivazione. Sempre redato dal \textit{Project Manager} schematizzato :
\begin{enumerate}
...
E' importante concordare con il cliente una visione del prodotto attesto.\\
...
Rispettivamente uno accerta la realizzabilità l'altro si accordo sulle caratteristiche sul prodotto da realizzare\\
...
Accentramento del soddisfacimento di tutti i requisiti utente pattuiti nella \textit{Revisione dei Requisiti}.\\
Prodotti esterni valutati :
\begin{itemize}
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item \textit{Businnes}: relatuvu a costi e concorrenza.
	\end{itemize}
...
\end{enumerate}
Ogni rischio identificato è stato descritto con: nome, probabilità di occorrenza, grado di pericolosità, descrizione, strategie per la rilevazione e contromisure. Ciascun rischio verrà nel tempo monitorato e ne verrà aggiornato l'effettivo riscontro con l'avanzamento del progetto nella seguente maxi-tabella:
...
\small
	\begin{tabular}{|c|c|c|c|c|c|}
		\hline 
		\textbf{Nome} & \textbf{Occorrenza} & \textbf{Pericolosità} & \textbf{Descrizione} & \textbf{Rilevazione} & \textbf{Contromisure} \\ 
		\hline 
		\multicolumn{6}{|c|}{\textbf{Livello Tecnologico}} \\  
		\textit{Tecnologie Adottate} & Media & Alta & \minitab[c]{Incompatibilità tra le tecnologie \\ adottate e le proprie conoscenze}  & Project Manager & \minitab[c]{Ciascun membro del team ha il dovere di informarsi con l'ausilio \\ di documentazione anche fornita dal Responsabile} \\ 
		\textit{Rottura Hardware} & Bassa & Bassa & \minitab[c]{Rottura del sistema di archiviazione \\ o degli strumenti per realizzazione del progetto} & Singolo Componente & \minitab[c]{Si risolve utilizzando strumenti forniti \\ dall'università in sostituzione a quelli personali} \\ 
		\hline 
		\multicolumn{6}{|c|}{\textbf{Livello Personale}} \\  
		\textit{Problemi dei singoli} & Media & Media & \minitab[c]{Impegni personali e necessità per altre \\materie sono all'ordine del giorno} & Singolo Componente & Subito dopo la comunicazione sarà il Project Manager a rielaborare le mansioni e l'organizzazione del gruppo \\  
		\textit{Probelmi tra componenti} & Bassa & Alta & \minitab[c]{Crearsi di un clima instabile all'interno del gruppo e\\ difficioltà di collaborazione} & Project Manager, Singolo Componente & Il Project Manager può ridistribuire i ruoli affinché non vi sia dipendenze tra i litiganti o dimettersi \\ 
		\textit{Inesperienza} & Alta & Alta & \minitab[c]{Inesperienza nell'uso della tecnologia e\\ conoscenza generica della progettazione e analisi} & Singolo Componente & Impegno del singolo per informarsi e aggiornarsi lasciando libertà di tempo aggiuntivo nel caso vi siano difficoltà \\ 
		\hline 
		\multicolumn{6}{|c|}{\textbf{Livello organizzativo e valutazione dei costi}} \\ 
		\textit{Valutazione Efficenza} & Alta & Media & Sbagliata la stima delle tempistiche e dei costi nel piano & Project Manager , Analista , Singolo Componente & Riprogettazione immediata nel piano organizzativo e, se necessario, cambiamento delle priorità \\ 
		\hline 
		\multicolumn{6}{|c|}{\textbf{Livello dei requisiti}} \\ 
		\textit{Comprensione Requisiti} & Media & Medio & \minitab[c]{Errata analisi dei requisiti e\\ visione diversa tra Analista e Proponente di un obbiettivo} & Progettista , Verificatore , Singolo Componente & Riunione e comunicazione tra le parti con correzzioni al piano \\ 
		\hline 
	\end{tabular} 
\normalsize
\end{landscape}
...
\begin{itemize}
	...
	\item Per specializzazione : a seconda di cui il singolo è specializzato ricopre un'attività inerente al suo ruolo. (Es: Esperto di Latex produce un template, ecc...)
	\item Per verifica : si dà per certezza la verifica di ogni documento e ogni passaggio di stesura. Quindi la verifica è assegnata ai componenti in cui non vi è conflitto di interesse sulla stesura da parte del \textit{Project Manager}.
\end{itemize} 
...
Le attività nella punto di Analisi sono:
\begin{enumerate}
	...
	durante le attività. Il rispetto di tali norme  sarà  poi compito dei verificatori accertarlo;
	\item \textbf{Analisi dei Requisiti}: viene fatta un'analisi approfondita partendo dalla base fatta durante 
	lo Studio di Fattibilità. Questa attività continuerà fino alla data di consegna;
	\item \textbf{Piano di Progetto}: il responsabile del gruppo redige il questo documento così da organizzare
	le attività del gruppo. Questa attività ha un'alta priorità;
	\item \textbf{Piano di Qualifica}: l'\textit{Analista} redige il Piano di Qualifica in collaborazione con l'\textit{Amministratore}ed il \textit{Responsabile di Progetto}; 
	\item \textbf{Glossario}: viene scritto in modo incrementale da chi redige i documenti. Contiene la 
	spiegazione di alcuni termini utilizzati. Viene redatto in parallelo a tutti i documenti ed 
	è aggiornato ad ogni termine che necessita di una spiegazione;
	\item \textbf{Lettera di presentazione}: documento presentato al committente che permette al gruppo 
	di partecipare alla gara d'appalto per il capitolato.
\end{enumerate}
...
\begin{itemize}
	...
	\item \textbf{Codifica}: in questa fase inizia lo sviluppo del codice da parte dei programmatori, 
	che devono seguire quanto riportato nel documento \textit{Definizione di Prodotto};
...
{\renewcommand\arraystretch{1.2}  %aumenta l'altezza di ogni riga
	\begin{tabular}{|c|c|c|c|c|c|c|c|}
		...
		\hline 
		Sebastiano Marchesini &  &  & 5x25 & 16x22 &  & 4x15 & 537,00 \\ 
		\hline 
		Gianluca Crivellaro & 3x30 &  & 2x25 & 17x22 &  & 1x15 & 538,00 \\ 
		\hline 
		Pietro Lonardi &  & 2x20 & 2x25 & 14x22 &  & 6x15 & 488,00 \\ 
		\hline 
		Alberto Gelmi & 3x30 &  & 4x25 & 21x22 &  &  & 652,00 \\ 
		\hline 
		Piergiorgio Danieli &  &  & 2x25 & 17x22 &  & 4x15 & 484,00 \\ 
		\hline 
		\multicolumn{7}{r|}{TOTALE  :} & \textit{\euro 2699,00} \\ 
\end{tabular}} 
...
{\renewcommand\arraystretch{1.2}  %aumenta l'altezza di ogni riga
	\begin{tabular}{|c|c|c|c|c|c|c|c|}
		...
		\hline
		Sebastiano Marchesini	& 10 & 9 & 10 & 38 & 30 & 8 & 105 \\ 
		\hline 
		Gianluca Crivellaro 	& 4 & 4 & 34 & 33 & 21 & 9 & 105 \\ 
		\hline 
		Pietro Lonardi 			& 3 & 3 & 18 & 34 & 5 & 42 & 105 \\ 
		\hline 
		Alberto Gelmi 			& 8 & 2 & 19 & 27 & 30 & 22 & 105 \\ 
		\hline 
		Piergiorgio Danieli 	& 6 & 5 & 4 & 39 & 29 & 22 & 105 \\ 
		\hline 
...
{\renewcommand\arraystretch{1.2}  %aumenta l'altezza di ogni riga
	\begin{tabular}{|c|c|c|c|c|c|c|c|}
		...
		& \textbf{PM} & \textbf{AM} & \textbf{AN} & \textbf{PL} & \textbf{PR} & \textbf{VE} & \\ 
		\hline
		Sebastiano Marchesini & 8 & 9 & 5 &  &  & 3 &  \\ 
		\hline 
		Gianluca Crivellaro & 1 &  & 22 &  &  & 3 & 6 \\ 
		\hline 
		Pietro Lonardi &  & 1 & 6 &  &  & 8 & 6 \\ 
		\hline 
		Alberto Gelmi &  &  &  &  &  &  & 5 \\ 
		\hline 
		Piergiorgio Danieli & 5 & 5 & 2 &  &  & 5 & 5 \\ 
		\hline 
		\multicolumn{7}{r|}{TOTALE  :} &  \\ 
\end{tabular}} 
